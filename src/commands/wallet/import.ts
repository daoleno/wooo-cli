...
export default defineCommand({
  ...
  args: {
    key: { type: "positional", description: "Private key (or use stdin/--file/interactive)", required: false },
  ...
  async run({ args }) {
    ...
    if (args.file) {
      ...
      console.error(ansis.yellow("⚠ Warning: Private key in CLI args is visible in shell history."));
      privateKey = args.key;
    } else if (process.stdin.isTTY) {
      const clack = await import("@clack/prompts");
      const value = await clack.password({ message: "Enter private key:" });
      if (!value || typeof value === "symbol") { console.error("No key provided"); process.exit(6); }
      privateKey = value;
    } else {
      const chunks: Buffer[] = [];
    ...
    out.data({ name: wallet.name, address: wallet.address, chain: wallet.chain });
  },
});
