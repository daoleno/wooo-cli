...
): WriteOperation<PreparedAaveSupply, WalletPort, AaveSupplyResult> {
  return {
    protocol: "aave",
    prepare: async () => {
...
): WriteOperation<PreparedAaveBorrow, WalletPort, AaveBorrowResult> {
  return {
    protocol: "aave",
    prepare: async () => {
      const client = new AaveClient(params.chain);
...
): WriteOperation<PreparedAaveWithdraw, WalletPort, AaveWithdrawResult> {
  return {
    protocol: "aave",
    prepare: async () => {
      const client = new AaveClient(params.chain);
...
): WriteOperation<PreparedAaveRepay, WalletPort, AaveRepayResult> {
  return {
    protocol: "aave",
    prepare: async () => {
      const client = new AaveClient(params.chain);
